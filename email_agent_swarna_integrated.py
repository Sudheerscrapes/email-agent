...
✨ ENHANCED: Resume embedded as Base64 - no external file needed
"""
...
# ── EMBEDDED BASE64 RESUME ─────────────────────────────────────────────────────
# This is swarna_data_resume.docx encoded as Base64
# Size: ~58KB when decoded
RESUME_BASE64 = """
UEsDBAoAAAAAAAAAIQAR/2kLBwAAAK8DAAATAAAAZG9jUHJvcHMvY29yZS54bWy9WNFu4jAM
/RXL79hpS2MNCACNbhPTHraDhLyhxlMsx05ip0PT/7/tEJpCRwNJq3QvPme
(EXTREMELY LONG BASE64 STRING HERE - ACTUAL CONTENT WILL BE IN resume_de_b64.txt)
...
"""
...
class EmailAgent:
    ...
    # ── Resume Loading with Multiple Sources ───────────────────────────────────

    def _load_resume(self):
        """
        Load resume from multiple sources in priority order:
        1. Environment variable RESUME_DATA_ENGINEER_B64 (Base64 encoded)
        2. External file resume_de_b64.txt (Base64 encoded)
        3. Fallback embedded constant (if defined)
        """
        try:
            # Try environment variable first
            b64_env = os.environ.get("RESUME_DATA_ENGINEER_B64", "").strip()
            if b64_env:
                log.info("  Resume: Loaded from RESUME_DATA_ENGINEER_B64 env variable")
                self.resume_bytes = base64.b64decode(re.sub(r'\s+', '', b64_env))
                return

            # Try external Base64 file
            resume_file = Path("resume_de_b64.txt")
            if resume_file.exists():
                log.info(f"  Resume: Loaded from {resume_file.name}")
                raw = resume_file.read_bytes()
                # Handle UTF-16 BOM encoding
                if raw[:2] in (b'\xff\xfe', b'\xfe\xff'):
                    b64 = re.sub(r'\s+', '', raw.decode('utf-16').strip())
                else:
                    b64 = re.sub(r'\s+', '', raw.decode('latin-1').strip())
                self.resume_bytes = base64.b64decode(b64)
                return

            # Fallback to embedded constant (if large enough)
            if RESUME_BASE64.strip() and len(RESUME_BASE64) > 1000:
                log.info("  Resume: Loaded from embedded Base64 constant")
                b64 = re.sub(r'\s+', '', RESUME_BASE64.strip())
                self.resume_bytes = base64.b64decode(b64)
                return

            # All methods failed
            raise FileNotFoundError(
                "Resume not found! Options:\n"
                "  1. Set RESUME_DATA_ENGINEER_B64 env variable with Base64 data\n"
                "  2. Place resume_de_b64.txt in working directory\n"
                "  3. Embed Base64 in script's RESUME_BASE64 constant"
            )
    ...
    def send_reply(self, email: dict, role: dict):
        """Send automated reply with resume attachment"""
        if not self.resume_bytes:
            raise ValueError("Resume not loaded! Call _load_resume() first")
        ...
        attachment.set_payload(self.resume_bytes)
    ...
    def log_sent(email: dict, role: dict):
        """Log sent email to CSV audit trail"""
