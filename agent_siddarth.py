"""
AI Email Agent - Siddarth (Senior Salesforce Developer)
Scans: sudheeritservices1@gmail.com (IMAP - Gmail)
Sends: sudheer@adeptscripts.com (SMTP - Zoho)
Replies to: Salesforce Developer, Apex, LWC, Agentforce, CPQ, Experience Cloud roles
...
12. BROAD IMAP search: "salesforce" and "sfdc" - catches ALL variants
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent_siddarth.log"), logging.StreamHandler()])
log = logging.getLogger(__name__)

# ══════════════════════════════════════════════════════════════════════════════
# DAILY DEDUP - Resets every day at midnight
# ══════════════════════════════════════════════════════════════════════════════
DEDUP_FILE = Path("logs") / "daily_replied_senders_siddarth.json"
MAX_DAILY_SENDS = 450
...
ROLES = [
    {
        ...
        "resume_file": "resume_siddarth_b64.txt",
        "cc_secret": "CC_SIDDARTH",
        "reply": SHARED_REPLY,
    },
    {
        "name": "Salesforce Developer (Apex / LWC)",
        "keywords": [
    ...
    {
        "name": "Salesforce Experience Cloud / Community Developer",
        "keywords": [
            "salesforce experience cloud developer",
            "salesforce community developer",
            "experience cloud developer",
            "salesforce community cloud",
            "lightning community developer",
            "salesforce portal developer",
            "salesforce lwr developer",
            "salesforce site developer",
        ],
        "resume_file": "resume_siddarth_b64.txt",
        "cc_secret": "CC_SIDDARTH",
        "reply": SHARED_REPLY,
    ...
    {
        "name": "Salesforce Integration / MuleSoft Developer",
        "keywords": [
            "salesforce integration developer",
            "salesforce mulesoft developer",
            "salesforce api developer",
            "salesforce rest api developer",
            "salesforce soap developer",
            "salesforce integration engineer",
            "mulesoft developer",
            "mulesoft engineer",
            "salesforce middleware developer",
            "salesforce platform events developer",
        ],
        "resume_file": "resume_siddarth_b64.txt",
        "cc_secret": "CC_SIDDARTH",
        "reply": SHARED_REPLY,
    ...
    {
        "name": "Salesforce Architect / Consultant",
        "keywords": [
...
# Fallback role: if subject has salesforce or sfdc but no keyword matched above
FALLBACK_ROLE = {
    "name": "Salesforce Developer (General)",
    "keywords": [],
    "resume_file": "resume_siddarth_b64.txt",
    "cc_secret": "CC_SIDDARTH",
    "reply": SHARED_REPLY,
...
REPLIED_LABEL = "AutoReplied_Siddarth"
...
def fetch_matching_emails():
    ...
    # ── BROAD SEARCH: "salesforce" and "sfdc" catch ALL variants ──
    all_uid_set = set()
    search_queries = [
        'SINCE "' + today + '" SUBJECT "salesforce"',
        'SINCE "' + today + '" SUBJECT "sfdc"',
    ]
    ...
    log.info("Found %d matching emails today (salesforce + sfdc)", len(ids))
...
def detect_role(email):
    ...
    # Fallback: if subject contains salesforce or sfdc, still send
    if "salesforce" in subject or "sfdc" in subject:
...
def main():
    log.info("=" * 70)
    log.info("AI Email Agent - Siddarth (Senior Salesforce Developer)")
    ...
    required = ["IMAP_EMAIL", "IMAP_APP_PASSWORD", "SIDDARTH_SMTP_EMAIL", "SIDDARTH_SMTP_APP_PASSWORD"]
    ...
    # Zoho SMTP settings
    smtp_email = os.environ["SIDDARTH_SMTP_EMAIL"]
    smtp_server = None
    try:
        smtp_server = smtplib.SMTP_SSL("smtp.zoho.com", 465)
        smtp_server.login(smtp_email, os.environ["SIDDARTH_SMTP_APP_PASSWORD"])
    ...
    for email in emails:
        ...
        try:
            ...
            if role is None:
                log.info("No salesforce / sfdc found in subject - skipping")
                continue
    ...
    log.info("SEND account : %s", os.environ.get("SIDDARTH_SMTP_EMAIL"))
