...
# ══════════════════════════════════════════════════════════════════════════════
DEDUP_FILE = Path("logs") / "daily_replied_senders_siva.json"
MAX_DAILY_SENDS = 450
...
ROLES = [
    {
        "name": "SAP BTP Integration Architect",
        "keywords": [
            "sap btp integration architect",
            "btp integration architect",
            "sap btp architect",
...
def fetch_matching_emails():
    ...
    search_queries = [
        ...
        'SINCE "' + today + '" SUBJECT "BTP"',
        'SINCE "' + today + '" SUBJECT "CPI"',
        'SINCE "' + today + '" SUBJECT "PI/PO"',
        'SINCE "' + today + '" SUBJECT "S/4 Hana"',
        'SINCE "' + today + '" SUBJECT "S4 Hana"',
        'SINCE "' + today + '" SUBJECT "ARIBA"',
    ]
...
def send_reply(email, role, server):
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_Siva_SAP_Integration.docx"')
    msg.attach(part)
...
def main():
    ...
    required = ["IMAP_EMAIL", "IMAP_APP_PASSWORD", "SIVA_SMTP_EMAIL", "SIVA_SMTP_APP_PASSWORD"]
