...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent2.log"), logging.StreamHandler()])
...
def send_reply(email, role, your_email, app_password):
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_Malathi_Gambiraopet.docx"')
    msg.attach(part)
    recipients = [to_email]
