"""
AI Email Agent - Govardhana Reddy Gonchi (SAP EWM / MM / SCM Consultant)
Scans: sudheeritservices1@gmail.com (IMAP - Gmail)
Sends: sudheer@adeptscripts.com (SMTP - Zoho)
Replies to: SAP EWM, SAP MM, SAP WM, SAP P2P, SAP S/4HANA roles (REMOTE ONLY)
...
12. BROAD IMAP search with remote filter at IMAP level
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent_govardhana.log"), logging.StreamHandler()])
log = logging.getLogger(__name__)

# ══════════════════════════════════════════════════════════════════════════════
# DAILY DEDUP - Resets every day at midnight
# ══════════════════════════════════════════════════════════════════════════════
DEDUP_FILE = Path("logs") / "daily_replied_senders_govardhana.json"
MAX_DAILY_SENDS = 450
...
ROLES = [
    {
        ...
        "keywords": [
            ...
            "sap extended warehouse management lead",
            "sap s/4hana ewm consultant",
            "sap s4hana ewm consultant",
            "s/4hana ewm consultant",
            "s4hana ewm consultant",
        ],
        "resume_file": "resume_govardhana_b64.txt",
        "cc_secret": "CC_GOVARDHANA",
        "reply": SHARED_REPLY,
    },
    {
        "name": "SAP MM / EWM Consultant",
        "keywords": [
            "sap mm/ewm consultant",
            "sap ewm/mm consultant",
            "sap mm ewm consultant",
            "sap ewm mm consultant",
            "sap mm and ewm consultant",
            "sap ewm and mm consultant",
            "sap mm/ewm lead",
            "sap ewm/mm lead",
    ...
    },
    {
        ...
        "keywords": [
            ...
            "sap materials management lead",
            "sap s/4hana mm consultant",
            "sap s4hana mm consultant",
            "s/4hana mm consultant",
        ],
        "resume_file": "resume_govardhana_b64.txt",
        "cc_secret": "CC_GOVARDHANA",
        "reply": SHARED_REPLY,
    },
    {
        ...
        "keywords": [
            ...
            "sap warehouse management lead",
            "sap wm/mm consultant",
            "sap mm/wm consultant",
            "sap wm functional consultant",
            "sap wm functional lead",
            "sap warehouse consultant",
            "sap wm architect",
        ],
        "resume_file": "resume_govardhana_b64.txt",
        "cc_secret": "CC_GOVARDHANA",
    ...
    {
        "name": "SAP P2P / Procure to Pay Consultant",
        "keywords": [
            "sap p2p consultant",
            "sap p2p lead",
            "sap procure to pay consultant",
            "sap procure-to-pay consultant",
            "sap purchase to pay consultant",
    ...
    {
        "name": "SAP Procurement / Supply Chain Consultant",
        "keywords": [
            "sap procurement consultant",
            "sap procurement lead",
            "sap purchasing consultant",
            "sap purchasing lead",
            "sap sourcing and procurement consultant",
            "sap sourcing consultant",
            "sap supply chain consultant",
            "sap scm consultant",
            "sap ariba mm consultant",
            "sap ariba procurement consultant",
            "sap indirect procurement consultant",
...
# Fallback: subject has sap ewm/mm/wm but no specific match
FALLBACK_ROLE = {
    "name": "SAP EWM/MM Consultant (General)",
    "keywords": [],
    "resume_file": "resume_govardhana_b64.txt",
    "cc_secret": "CC_GOVARDHANA",
    "reply": SHARED_REPLY,
...
REPLIED_LABEL = "AutoReplied_Govardhana"
...
def fetch_matching_emails():
    ...
    all_uid_set = set()
    search_queries = [
        'SINCE "' + today + '" SUBJECT "sap ewm" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap mm" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap wm" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap p2p" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap procurement" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap purchasing" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "procure to pay" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap s/4hana ewm" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap s4hana ewm" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "warehouse management" SUBJECT "remote"',
    ]
...
def detect_role(email):
    ...
    # Fallback
    fallback_terms = ["sap ewm", "sap mm", "sap wm", "sap p2p", "sap procurement",
                      "sap purchasing", "procure to pay", "warehouse management"]
...
def send_reply(email, role, server):
    smtp_email = os.environ["GOVARDHANA_SMTP_EMAIL"]
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_Govardhana_SAP_EWM_MM.docx"')
    msg.attach(part)
...
def log_sent(email, role):
    csv_path = "logs/sent_log_govardhana.csv"
...
def main():
    log.info("=" * 70)
    log.info("AI Email Agent - Govardhana Reddy Gonchi (SAP EWM / MM / SCM)")
    ...
    required = ["IMAP_EMAIL", "IMAP_APP_PASSWORD", "GOVARDHANA_SMTP_EMAIL", "GOVARDHANA_SMTP_APP_PASSWORD"]
    ...
    try:
        ...
        smtp_server.login(smtp_email, os.environ["GOVARDHANA_SMTP_APP_PASSWORD"])
    ...
    for email in emails:
        ...
        try:
            ...
            if role is None:
                log.info("No matching remote SAP EWM/MM role in subject - skipping")
                continue
    ...
    log.info("SEND account : %s", os.environ.get("GOVARDHANA_SMTP_EMAIL"))
