"""
AI Email Agent - Naga Ravi Kumar (SAP S/4HANA Sales / OTC / SD Consultant)
Scans: sudheeritservices1@gmail.com (IMAP - Gmail)
Sends: sudheer@adeptscripts.com (SMTP - Zoho)
Replies to: SAP SD, SAP OTC, SAP S/4HANA Sales, SAP Pricing, SAP Billing roles (REMOTE ONLY)
...
12. BROAD IMAP search: "sap sd", "sap otc", "sap s/4hana sales" - catches ALL variants
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent_nagaravi.log"), logging.StreamHandler()])
log = logging.getLogger(__name__)

# ══════════════════════════════════════════════════════════════════════════════
# DAILY DEDUP - Resets every day at midnight
# ══════════════════════════════════════════════════════════════════════════════
DEDUP_FILE = Path("logs") / "daily_replied_senders_nagaravi.json"
MAX_DAILY_SENDS = 450
...
ROLES = [
    {
        "name": "SAP S/4HANA Sales Consultant",
        "keywords": [
            "sap s/4hana sales consultant",
            "sap s/4hana sales lead",
            ...
            "sap s/4hana sales architect",
            "sap s4hana sales lead",
            "sap s/4hana sd consultant",
            "s4 hana sales consultant",
        ],
        "resume_file": "resume_nagaravi_b64.txt",
        "cc_secret": "CC_NAGARAVI",
        "reply": SHARED_REPLY,
    },
    {
    ...
    {
        "name": "SAP Pricing / Billing Consultant",
        "keywords": [
    ...
    {
        "name": "SAP OTC Lead / Senior Consultant",
        "keywords": [
            "sap otc lead consultant",
            "sap otc senior consultant",
            "sap otc sr consultant",
            "lead sap otc consultant",
            "senior sap otc consultant",
            "sap otc tpm",
            "sap otc functional consultant",
            "sap otc functional lead",
            "sap tpm otc",
            "sap order management consultant",
            "sap order management lead",
        ],
        "resume_file": "resume_nagaravi_b64.txt",
        "cc_secret": "CC_NAGARAVI",
        "reply": SHARED_REPLY,
    ...
    {
        "name": "SAP SD / CRM / FSCM Consultant",
        "keywords": [
            ...
            "sap sd/fscm consultant",
            "sap collections management consultant",
    ...
    {
        "name": "SAP ARIBA / EDI / IDoc Consultant",
        "keywords": [
            "sap ariba sd consultant",
            "sap edi sd consultant",
            "sap idoc sd consultant",
            "sap ale edi consultant",
            "sap edi idoc consultant",
            "sap b2b sd consultant",
...
# Fallback: subject has sap sd / otc keywords but no specific match
FALLBACK_ROLE = {
...
REPLIED_LABEL = "AutoReplied_NagaRavi"
...
def fetch_matching_emails():
    ...
    # ── BROAD SEARCH: catches all SAP SD/OTC/S4HANA Sales variants ──
    all_uid_set = set()
    search_queries = [
        'SINCE "' + today + '" SUBJECT "sap sd"',
        'SINCE "' + today + '" SUBJECT "sap otc"',
        'SINCE "' + today + '" SUBJECT "sap s/4hana sales"',
        'SINCE "' + today + '" SUBJECT "sap s4hana sales"',
        'SINCE "' + today + '" SUBJECT "order to cash"',
    ]
    ...
    log.info("Found %d matching emails today (sap sd + otc + s4hana sales + order to cash)", len(ids))
...
def detect_role(email):
    ...
    # Fallback: subject has sap sd/otc keywords but no specific match
    fallback_terms = ["sap sd", "sap otc", "sap s/4hana sales", "sap s4hana sales", "order to cash"]
...
def send_reply(email, role, server):
    smtp_email = os.environ["NAGARAVI_SMTP_EMAIL"]
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_NagaRavi_SAP_SD_OTC.doc"')
    msg.attach(part)
...
def log_sent(email, role):
    csv_path = "logs/sent_log_nagaravi.csv"
...
def main():
    log.info("=" * 70)
    log.info("AI Email Agent - Naga Ravi Kumar (SAP S/4HANA Sales / OTC / SD)")
    ...
    required = ["IMAP_EMAIL", "IMAP_APP_PASSWORD", "NAGARAVI_SMTP_EMAIL", "NAGARAVI_SMTP_APP_PASSWORD"]
    ...
    try:
        ...
        smtp_server.login(smtp_email, os.environ["NAGARAVI_SMTP_APP_PASSWORD"])
    ...
    log.info("SEND account : %s", os.environ.get("NAGARAVI_SMTP_EMAIL"))
