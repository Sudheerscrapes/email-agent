"""
AI Email Agent - Vamsee M (SAP SD/OTC/S4HANA/Vistex/RAR Consultant)
Scans: sudheeritservices1@gmail.com (IMAP - Gmail)
Sends: sudheer@adeptscripts.com (SMTP - Zoho)
Replies to: SAP SD, OTC, S/4HANA, Vistex, RAR, Pricing, Billing roles (REMOTE ONLY)
...
12. BROAD IMAP search: "sap sd", "sap otc", "vistex", "sap rar" - catches ALL variants
13. REMOVED UNSEEN filter - catches read and unread emails
14. REMOTE ONLY - skips non-remote roles
"""
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent_vamsee.log"), logging.StreamHandler()])
log = logging.getLogger(__name__)

# ══════════════════════════════════════════════════════════════════════════════
# DAILY DEDUP - Resets every day at midnight
# ══════════════════════════════════════════════════════════════════════════════
DEDUP_FILE = Path("logs") / "daily_replied_senders_vamsee.json"
MAX_DAILY_SENDS = 450
...
ROLES = [
    ...
    {
        "name": "SAP S/4HANA Sales / OTC Consultant",
        "keywords": [
            "sap s/4hana sales consultant",
            "sap s4hana sales consultant",
            "sap s/4hana otc consultant",
            "sap s4hana otc consultant",
            "sap s/4hana sd consultant",
            "sap s4hana sd consultant",
            "s/4hana sales consultant",
            "s4hana sales consultant",
            "sap s/4hana order to cash",
            "sap s4 hana sales consultant",
            "sap s/4 hana sales consultant",
        ],
        "resume_file": "resume_vamsee_b64.txt",
        "cc_secret": "CC_VAMSEE",
        "reply": SHARED_REPLY,
    },
    {
        ...
        "keywords": [
            ...
            "vistex developer",
            "vistex architect",
            "sap vistex developer",
            "sap vistex architect",
            "vistex incentives consultant",
            "vistex rebates consultant",
            "sap vistex incentives",
            "sap vistex rebates",
            "vistex payback consultant",
            "vistex dmp consultant",
        ],
        "resume_file": "resume_vamsee_b64.txt",
        "cc_secret": "CC_VAMSEE",
        "reply": SHARED_REPLY,
    },
    {
        ...
        "keywords": [
            "sap rar consultant",
            "sap rar lead",
            "sap revenue accounting consultant",
            "sap revenue accounting and reporting",
            "sap revenue recognition consultant",
            "sap asc 606 consultant",
            "sap rar developer",
            "sap revenue accounting lead",
            "revenue accounting consultant",
            "sap rar/sd consultant",
        ],
        "resume_file": "resume_vamsee_b64.txt",
        "cc_secret": "CC_VAMSEE",
        "reply": SHARED_REPLY,
    ...
    {
        "name": "SAP Pricing / Rebates Consultant",
        "keywords": [
            "sap pricing consultant",
            "sap pricing lead",
            "sap sd pricing consultant",
            "sap rebates consultant",
            "sap rebates lead",
            "sap condition contract consultant",
            "sap ccm consultant",
            "sap settlement management consultant",
            "sap incentive management consultant",
            "sap sd rebates consultant",
            "sap pricing and rebates",
            "sap rebate management consultant",
        ],
        "resume_file": "resume_vamsee_b64.txt",
        "cc_secret": "CC_VAMSEE",
        "reply": SHARED_REPLY,
    ...
    {
        "name": "SAP BRIM / Subscription Order Management Consultant",
        "keywords": [
            "sap brim consultant",
            "sap brim lead",
            "sap subscription order management",
            "sap som consultant",
            "sap brim/sd consultant",
            "sap convergent invoicing",
            "sap ci consultant",
            "sap brim developer",
            "sap subscription billing consultant",
            "sap brim architect",
        ],
        "resume_file": "resume_vamsee_b64.txt",
        "cc_secret": "CC_VAMSEE",
    ...
    {
        "name": "SAP QTC / Quote to Cash Consultant",
        "keywords": [
            "sap qtc consultant",
            "sap quote to cash consultant",
            "sap quote-to-cash consultant",
            "sap qtc lead",
            "sap cpq consultant",
            "sap sd cpq consultant",
            "quote to cash consultant",
            "qtc consultant",
            "sap qtc/otc consultant",
        ],
        "resume_file": "resume_vamsee_b64.txt",
        "cc_secret": "CC_VAMSEE",
        "reply": SHARED_REPLY,
...
# Fallback: subject has sap sd/otc/vistex but no specific match
FALLBACK_ROLE = {
    "name": "SAP SD/OTC Consultant (General)",
    "keywords": [],
    "resume_file": "resume_vamsee_b64.txt",
    "cc_secret": "CC_VAMSEE",
    "reply": SHARED_REPLY,
...
REPLIED_LABEL = "AutoReplied_Vamsee"
...
def fetch_matching_emails():
    ...
    # ── BROAD SEARCH + REMOTE filter at IMAP level ──
    all_uid_set = set()
    search_queries = [
        'SINCE "' + today + '" SUBJECT "sap sd" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap otc" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap s/4hana sales" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap s4hana sales" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "vistex" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap rar" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "order to cash" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "sap brim" SUBJECT "remote"',
        'SINCE "' + today + '" SUBJECT "quote to cash" SUBJECT "remote"',
    ]
    ...
    log.info("Found %d remote matching emails today", len(ids))
...
def detect_role(email):
    ...
    if not any(rk in subject for rk in remote_keywords):
        log.info("SKIPPING - not a remote role: %s", subject[:60])
        return None
    ...
    # Fallback: subject has sap sd/otc/vistex/rar but no specific match
    fallback_terms = ["sap sd", "sap otc", "vistex", "sap rar", "order to cash", "sap brim", "quote to cash"]
    if any(term in subject for term in fallback_terms):
...
def send_reply(email, role, server):
    smtp_email = os.environ["VAMSEE_SMTP_EMAIL"]
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_Vamsee_SAP_SD_OTC.docx"')
    msg.attach(part)
...
def log_sent(email, role):
    csv_path = "logs/sent_log_vamsee.csv"
...
def main():
    ...
    required = ["IMAP_EMAIL", "IMAP_APP_PASSWORD", "VAMSEE_SMTP_EMAIL", "VAMSEE_SMTP_APP_PASSWORD"]
    ...
    try:
        ...
        smtp_server.login(smtp_email, os.environ["VAMSEE_SMTP_APP_PASSWORD"])
    ...
    for email in emails:
        ...
        try:
            ...
            if role is None:
                log.info("No matching remote SAP SD/OTC role in subject - skipping")
                continue
    ...
    log.info("SEND account : %s", os.environ.get("VAMSEE_SMTP_EMAIL"))
