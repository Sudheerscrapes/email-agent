"""
AI Email Agent - Lovkya (SAP EWM/IM Functional Solution Architect)
Scans: sudheeritservices1@gmail.com (IMAP - Gmail)
Sends: sudheer@adeptscripts.com (SMTP - Zoho)
Replies to: SAP EWM, SAP WM, SAP IM, SAP SCM, S/4HANA Warehouse roles
...
12. BROAD IMAP search: "ewm" and "warehouse" - catches ALL variants
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent_lovkya.log"), logging.StreamHandler()])
log = logging.getLogger(__name__)

# ══════════════════════════════════════════════════════════════════════════════
# DAILY DEDUP - Resets every day at midnight
# ══════════════════════════════════════════════════════════════════════════════
DEDUP_FILE = Path("logs") / "daily_replied_senders_lovkya.json"
MAX_DAILY_SENDS = 450
...
ROLES = [
    {
        "name": "SAP EWM Solution Architect",
        "keywords": [
            "sap ewm solution architect",
            "sap ewm architect",
            "ewm solution architect",
            "sap extended warehouse management architect",
            "sap ewm functional architect",
            "sap ewm lead architect",
            "sap ewm senior architect",
            "warehouse management architect",
        ],
        "resume_file": "resume_lovkya_b64.txt",
        "cc_secret": "CC_LOVKYA",
        "reply": SHARED_REPLY,
    },
    {
        "name": "SAP EWM Functional Consultant",
        "keywords": [
    ...
    {
        "name": "SAP EWM / WM Consultant",
        "keywords": [
            "sap ewm/wm",
            "sap wm/ewm",
            "sap ewm wm consultant",
            "sap wm ewm",
            "sap warehouse management consultant",
            "sap wm consultant",
            "sap wm functional",
            "sap wm lead",
            "sap warehouse management functional",
    ...
    {
        "name": "SAP EWM S/4HANA Consultant",
        "keywords": [
            "sap ewm s/4hana",
            "sap s/4hana ewm",
            "s4hana ewm",
            ...
            "rise with sap ewm",
            "sap ewm cloud",
            "sap s/4hana warehouse",
            "s/4hana warehouse management",
            "embedded ewm",
            "sap embedded ewm",
        ],
        "resume_file": "resume_lovkya_b64.txt",
    ...
    {
        "name": "SAP EWM / IM Consultant",
        "keywords": [
            "sap ewm/im",
            "sap im/ewm",
            "sap ewm im consultant",
            "sap inventory management ewm",
            "sap im consultant",
    ...
    },
    {
        ...
        "keywords": [
            ...
            "sap ewm pp consultant",
            "sap ewm qm consultant",
            "sap warehouse logistics",
            "sap scm functional",
...
# Fallback: if subject has ewm or warehouse management keywords
FALLBACK_ROLE = {
    "name": "SAP EWM Consultant (General)",
    "keywords": [],
    "resume_file": "resume_lovkya_b64.txt",
    "cc_secret": "CC_LOVKYA",
    "reply": SHARED_REPLY,
...
REPLIED_LABEL = "AutoReplied_Lovkya"
...
def fetch_matching_emails():
    ...
    # ── BROAD SEARCH: "ewm" and "warehouse" catch ALL variants ──
    all_uid_set = set()
    search_queries = [
        'SINCE "' + today + '" SUBJECT "ewm"',
        'SINCE "' + today + '" SUBJECT "warehouse management"',
    ]
    ...
    log.info("Found %d matching emails today (ewm + warehouse management)", len(ids))
...
def detect_role(email):
    ...
    # Fallback: if subject contains ewm or warehouse management
    if "ewm" in subject or "warehouse management" in subject:
...
def send_reply(email, role, server):
    smtp_email = os.environ["LOVKYA_SMTP_EMAIL"]  # sudheer@adeptscripts.com
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_Lovkya_SAP_EWM.docx"')
    msg.attach(part)
...
def log_sent(email, role):
    csv_path = "logs/sent_log_lovkya.csv"
...
def main():
    log.info("=" * 70)
    log.info("AI Email Agent - Lovkya (SAP EWM/IM Functional Solution Architect)")
    ...
    required = ["IMAP_EMAIL", "IMAP_APP_PASSWORD", "LOVKYA_SMTP_EMAIL", "LOVKYA_SMTP_APP_PASSWORD"]
    ...
    # Zoho SMTP settings
    smtp_email = os.environ["LOVKYA_SMTP_EMAIL"]
    smtp_server = None
    try:
        smtp_server = smtplib.SMTP_SSL("smtp.zoho.com", 465)
        smtp_server.login(smtp_email, os.environ["LOVKYA_SMTP_APP_PASSWORD"])
    ...
    for email in emails:
        ...
        try:
            ...
            if role is None:
                log.info("No ewm / warehouse management found in subject - skipping")
                continue
    ...
    log.info("SEND account : %s", os.environ.get("LOVKYA_SMTP_EMAIL"))
