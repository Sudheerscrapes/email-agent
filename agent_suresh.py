"""
AI Email Agent - Suresh Akula (Network Engineer - Cisco / BGP / OSPF / AWS / VMware NSX)
Scans: sudheeritservices1@gmail.com (IMAP - Gmail)
Sends: via Gmail SMTP
Replies to: Network Engineer, Network Admin, NOC Engineer, Cloud Network, Cisco roles
NO remote filter - replies to ALL matching roles
...
12. BROAD IMAP search - catches ALL network engineer variants
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent_suresh.log"), logging.StreamHandler()])
log = logging.getLogger(__name__)

# ══════════════════════════════════════════════════════════════════════════════
# DAILY DEDUP - Resets every day at midnight
# ══════════════════════════════════════════════════════════════════════════════
DEDUP_FILE = Path("logs") / "daily_replied_senders_suresh.json"
MAX_DAILY_SENDS = 450
...
ROLES = [
    {
        ...
        "resume_file": "resume_suresh_b64.txt",
        "cc_secret": "CC_SURESH",
        "reply": SHARED_REPLY,
    },
    {
    ...
    {
        "name": "NOC Engineer / Network Operations",
        "keywords": [
            "noc engineer",
            "noc analyst",
    ...
    {
        "name": "VMware / NSX Network Engineer",
        "keywords": [
            ...
            "vmware nsxt engineer",
            "nsx-v engineer",
            "vmware networking engineer",
            "nsx network architect",
        ],
        "resume_file": "resume_suresh_b64.txt",
        "cc_secret": "CC_SURESH",
        "reply": SHARED_REPLY,
    ...
    {
        "name": "Routing & Switching Engineer",
        "keywords": [
...
# Fallback: subject has network engineer/admin but no specific match
FALLBACK_ROLE = {
    "name": "Network Engineer (General)",
    "keywords": [],
    "resume_file": "resume_suresh_b64.txt",
    "cc_secret": "CC_SURESH",
    "reply": SHARED_REPLY,
...
REPLIED_LABEL = "AutoReplied_Suresh"
...
def fetch_matching_emails():
    ...
    # ── BROAD SEARCH: catches all network engineer variants ──
    all_uid_set = set()
    search_queries = [
        'SINCE "' + today + '" SUBJECT "network engineer"',
        'SINCE "' + today + '" SUBJECT "network administrator"',
        'SINCE "' + today + '" SUBJECT "noc engineer"',
        'SINCE "' + today + '" SUBJECT "cloud network engineer"',
        'SINCE "' + today + '" SUBJECT "network security engineer"',
        'SINCE "' + today + '" SUBJECT "network automation engineer"',
        'SINCE "' + today + '" SUBJECT "vmware nsx"',
        'SINCE "' + today + '" SUBJECT "routing and switching"',
        'SINCE "' + today + '" SUBJECT "network operations engineer"',
        'SINCE "' + today + '" SUBJECT "cisco network"',
    ]
...
def detect_role(email):
    ...
    # Fallback: subject has network-related keywords but no specific match
    fallback_terms = ["network engineer", "network admin", "noc engineer", "network operations",
                      "cisco network", "cloud network", "routing switching", "network security"]
...
def send_reply(email, role, server):
    smtp_email = os.environ["SURESH_SMTP_EMAIL"]  # Gmail
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_Suresh_Akula_Network_Engineer.docx"')
    msg.attach(part)
...
def log_sent(email, role):
    csv_path = "logs/sent_log_suresh.csv"
...
def main():
    log.info("=" * 70)
    log.info("AI Email Agent - Suresh Akula (Network Engineer)")
    log.info("SCAN inbox : sudheeritservices1@gmail.com (Gmail IMAP)")
    log.info("SEND from  : Gmail SMTP")
    log.info("FILTER     : All matching roles (no remote filter)")
    ...
    required = ["IMAP_EMAIL", "IMAP_APP_PASSWORD", "SURESH_SMTP_EMAIL", "SURESH_SMTP_APP_PASSWORD"]
    ...
    # Gmail SMTP settings
    smtp_email = os.environ["SURESH_SMTP_EMAIL"]
    smtp_server = None
    try:
        smtp_server = smtplib.SMTP_SSL("smtp.gmail.com", 465)
        smtp_server.login(smtp_email, os.environ["SURESH_SMTP_APP_PASSWORD"])
    ...
    for email in emails:
        ...
        try:
            ...
            if role is None:
                log.info("No matching network role in subject - skipping")
                continue
    ...
    log.info("SEND account : %s", os.environ.get("SURESH_SMTP_EMAIL"))
